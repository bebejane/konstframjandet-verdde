...
const writeBarrell = (filePath, imports, preContent = "") => {
	...
	const content = imports
		.map(
			({ path, exports }) =>
				`export { ${exports
					.map(({ func, isDefault }) => (isDefault ? `default as ${func}` : func))
					.join(`, `)} } from '${path}'`
		)
		.join("\n");
	...
	glob(`${baseDir}/**/*.tsx`, {}, function (er, files) {
		...
		files.forEach((file) => {
			...
			fs.readFileSync(file, "utf8")
				.split("\n")
				.forEach((line) => {
					if (!line.trim().startsWith("export") || line.trim().startsWith("export type")) return;
			...
			if (path.endsWith("/index")) path = path.substring(0, path.lastIndexOf("/index"));
		...
		writeBarrell(
			`${baseDir}/index.ts`,
			imports.sort((a, b) => a.path > b.path)
		);
	});
};
...
const parseGraphql = () => {
	...
	files.forEach((file) => {
		...
		fs.readFileSync(`${baseDir}/${file}`, "utf8")
			.split("\n")
			.forEach((line) => {
